...
// Luces examen;
GLfloat	cutoff_aux = {20};
GLfloat desplazar = 0.7;


void dibuja (void){
	GLfloat vertices[] = {-altura*desplazar, -altura*desplazar , anchura,
	                      altura*desplazar , -altura*desplazar , anchura,
	                      altura*desplazar, altura*desplazar, anchura,
	                      -altura*desplazar, altura*desplazar, anchura};
...
void luces() {
	if(ilu){			//NOTE Modificar posicion según teclas
		GLfloat posicion[4] 	= {0.0, pos_y, 0.0, 1.0},
		        ambiente[4] 	= {0.2,0.2,0.2,1.0},
						intensidad[4] = {1.0,1.0,1.0,1.0};

		GLfloat direccion[3] = {0,-1.0,0};
		GLfloat	exponente = 70;
		...
		glLightf(GL_LIGHT2, GL_SPOT_CUTOFF, cutoff_aux);
	}
}
...
void normal_keys(unsigned char Tecla1,int x,int y){
	...
	if(cutoff_aux < 10) cutoff_aux=10;
	if(cutoff_aux > 100) cutoff_aux=100;
